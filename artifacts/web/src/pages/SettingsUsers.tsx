...
function NewUserModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await apiFetch("/api/admin/org/users", {
        method: "POST",
      ...
      <div className="bg-card border border-border rounded-xl shadow-2xl w-full max-w-sm">
        <div className="p-6 text-center space-y-4">
          ...
          <p className="text-sm text-muted-foreground">
            ...
            Share this temporary password with them securely:
          </p>
...
export default function SettingsUsers() {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (membershipId: string) => {
      const res = await apiFetch(`/api/admin/org/users/${membershipId}`, { method: "DELETE" });
      const data = await res.json();
  ...
  const updateRoleMutation = useMutation({
    mutationFn: async ({ membershipId, role }: { membershipId: string; role: string }) => {
      const res = await apiFetch(`/api/admin/org/users/${membershipId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        ...
        {membersQuery.isLoading ? (
          ...
          <div className="bg-card border border-border rounded-xl overflow-hidden">
            <table className="w-full">
              ...
              <tbody>
                {members.map((m) => {
                  const canModify = m.role !== "SUPER_ADMIN";
