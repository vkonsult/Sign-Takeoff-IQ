...
// ── Code-proximity pair extractor ─────────────────────────────────────────────

/**
 * A text label found spatially adjacent to a sign callout code on a floor plan.
 */
export interface CodeProximityPair {
  label: string;
  code: string;
  x: number;   // normalized centre x of the label phrase
  y: number;   // normalized centre y of the label phrase
  page: number;
}

/**
 * Scan all phrases on a floor plan page and return label/code pairs where a
 * sign callout code (e.g. "A-101", "S-3") is spatially adjacent to a text
 * label that is longer than 3 characters (or a known 3-character abbreviation).
 *
 * A "code token" matches the pattern: 1–4 uppercase letters + dash + 1–4 digits
 * (e.g. "A-101", "D-2", "AB-12", "EX-001").
 *
 * Proximity thresholds (in page points at scale 1.0):
 *   - Vertical:   centres within 25 pts  (same visual row)
 *   - Horizontal: centres within 250 pts (adjacent column)
 *
 * Title-block phrases are excluded (same zone as classification).
 */
export function extractCodeProximityPairs(
  pw: PageWords,
  pageNum: number,
  knownAbbreviations: Set<string> = KNOWN_ROOM_ABBREVIATIONS,
): CodeProximityPair[] {
  ...
  // Exclude title-block phrases.
  const drawingPhrases = phrases.filter((p) => !isInTitleBlockZone(p));

  // Code token: 1–4 uppercase letters + dash + 1–4 digits
  const CODE_TOKEN_RE = /^[A-Z]{1,4}-\d{1,4}$/;
  ...
  const codeTokens: PhraseInfo[] = [];
  const labelCandidates: PhraseInfo[] = [];
  ...
  for (const p of drawingPhrases) {
    ...
    const nx = (p.x0 + p.x1) / 2;
    const ny = (p.y0 + p.y1) / 2;
    const cx = nx * pageWidth;
    const cy = ny * pageHeight;
    const info: PhraseInfo = { text, nx, ny, cx, cy };

    const upper = text.toUpperCase();
    if (CODE_TOKEN_RE.test(upper)) {
      codeTokens.push({ ...info, text: upper });
      continue;
    }
    ...
    if (
      len > 4 ||
      (len === 3 && knownAbbreviations.has(text.toLowerCase()))
    ) {
      labelCandidates.push(info);
    }
  }

  if (codeTokens.length === 0 || labelCandidates.length === 0) return [];

  const PROX_Y_PTS = 25;
  const PROX_X_PTS = 250;

  const pairs: CodeProximityPair[] = [];

  for (const label of labelCandidates) {
    let nearest: { code: PhraseInfo; dist: number } | null = null;

    for (const code of codeTokens) {
      const dy = Math.abs(label.cy - code.cy);
      if (dy > PROX_Y_PTS) continue;
      const dx = Math.abs(label.cx - code.cx);
      if (dx > PROX_X_PTS) continue;
