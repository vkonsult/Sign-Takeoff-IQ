...
export async function runPdfProcessor(jobId: string): Promise<void> {
  ...
  // ── Preserve verified + manually-added + AI-extracted signs ──────────────
  // AI-extracted signs (data_source = 'ai') are preserved across rescans because
  // they were intentionally generated by the user via the on-demand AI scan.
  // Only heuristic/code-proximity (data_source = 'pdf') signs are cleared and re-run.
  ...
  const preservedSigns = existingSigns.filter(
    (s) => s.userVerified || s.manuallyAdded || s.dataSource === "ai"
  );

  logger.info({ jobId, preservedCount: preservedSigns.length }, "[PDF Processor] Preserved verified/manually-added/AI signs");

  // Delete only heuristic/pdf auto-extracted signs; keep user-verified, manually-added, and AI-extracted
  await db
    ...
    .where(
      and(
        ...
        ne(extractedSignsTable.dataSource, "ai")
      )
    );
