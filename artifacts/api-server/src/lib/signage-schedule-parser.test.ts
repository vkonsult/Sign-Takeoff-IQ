...
const ROTATED_FIXTURE_PATH = join(
  dirname(fileURLToPath(import.meta.url)),
  "__fixtures__",
  "sign-schedule-rotated.pdf",
);
const ROTATED_FIXTURE_FILE_ID = "test-sign-schedule-rotated";
...
describe("Real PDF fixture — full pipeline integration", () => {
  ...
  it("extractRawPageItems returns items with text and plausible viewport coordinates", async () => {
    ...
    // We scope the "1A" search to items BELOW the header so that a legend "1A"
    // earlier on the page does not satisfy the selector.
    const headerItem = items.find((it) => it.text === "SIGNAGE SCHEDULE")!;
    expect(headerItem).toBeDefined();
    const signCodeItem = items.find((it) => it.text === "1A" && it.y > headerItem.y)!;
...
// ── Rotated PDF fixture (/Rotate = 90) ─────────────────────────────────────────
//
// Architectural drawing sheets are frequently saved with /Rotate = 90 or 270.
// The coordinate-normalisation path in pdf-words.ts that calls
// convertToViewportPoint must handle the axis-swap that happens on those pages.
// This suite loads a fixture that was generated with page.setRotation(90°) and
// verifies:
//   1. The viewport dimensions are swapped (landscape: 792 × 612).
//   2. All normalised phrase coordinates are in [0, 1].
//   3. Phrases are ordered top-to-bottom (y0 increases monotonically on average),
//      confirming that the rotation-adjusted y-axis is correct.
//   4. The visual left-to-right column ordering is preserved, so the schedule
//      parser still assigns sign-code / qty / text to the right columns.
//   5. The full pipeline produces the same schedule entries and specs as the
//      unrotated fixture.
//
// The fixture PDF (sign-schedule-rotated.pdf) was generated by
// __fixtures__/generate-fixture.mjs using pdf-lib with page.setRotation(90°)
// and drawText rotate: degrees(90).  The 90° glyph rotation makes text advance
// in user +y space, which convertToViewportPoint maps to viewport +x (rightward),
// exactly as real CAD tools encode rotated-sheet text.  Placement formula:
//   y_pdf = vis_x        → viewport left edge = vis_x
//   x_pdf = vis_y + S/2  → viewport row centre ≈ vis_y  (S = font size)

describe("Rotated PDF fixture (/Rotate = 90) — full pipeline integration", () => {
  beforeAll(async () => {
    (globalThis as Record<string, unknown>)["require"] = _req;
    const { __resetPdfjsLibForTesting } = await import("./pdf-words");
    __resetPdfjsLibForTesting();
  ...
  it("returns landscape viewport dimensions (width=792, height=612) for a /Rotate=90 page", async () => {
    const { extractPagePhrases } = await import("./pdf-words");
    const pageWords = await extractPagePhrases(ROTATED_FIXTURE_PATH, ROTATED_FIXTURE_FILE_ID, 1);

    // A portrait PDF page (612×792) with /Rotate=90 is displayed as landscape.
    // pdfjs reports the visual dimensions, so width and height are swapped.
    expect(pageWords.pageWidth).toBeCloseTo(792, 0);
    expect(pageWords.pageHeight).toBeCloseTo(612, 0);
  });

  it("extracts at least one phrase per content row from the rotated fixture", async () => {
    const { extractPagePhrases } = await import("./pdf-words");
    const pageWords = await extractPagePhrases(ROTATED_FIXTURE_PATH, ROTATED_FIXTURE_FILE_ID, 1);

    // The rotated fixture has the same 11 text items as the unrotated one.
    expect(pageWords.phrases.length).toBeGreaterThanOrEqual(9);
  });

  it("all normalised phrase coordinates are within [0, 1] on the rotated page", async () => {
    const { extractPagePhrases } = await import("./pdf-words");
    const { phrases } = await extractPagePhrases(ROTATED_FIXTURE_PATH, ROTATED_FIXTURE_FILE_ID, 1);

    for (const p of phrases) {
      expect(p.x0).toBeGreaterThanOrEqual(0);
      expect(p.x0).toBeLessThanOrEqual(1);
      expect(p.x1).toBeGreaterThanOrEqual(0);
      expect(p.x1).toBeLessThanOrEqual(1);
      expect(p.y0).toBeGreaterThanOrEqual(0);
      expect(p.y0).toBeLessThanOrEqual(1);
      expect(p.y1).toBeGreaterThanOrEqual(0);
      expect(p.y1).toBeLessThanOrEqual(1);
      // Canonical order: x0 ≤ x1, y0 ≤ y1
      expect(p.x0).toBeLessThanOrEqual(p.x1);
      expect(p.y0).toBeLessThanOrEqual(p.y1);
    }
  });

  it("phrases are ordered top-to-bottom (y0 non-decreasing) on the rotated page", async () => {
    const { extractPagePhrases } = await import("./pdf-words");
    const { phrases } = await extractPagePhrases(ROTATED_FIXTURE_PATH, ROTATED_FIXTURE_FILE_ID, 1);

    // Phrases are emitted in the visual top-to-bottom order that the phrase
    // grouping loop produces.  A /Rotate=90 bug that fails to swap the y-axis
    // would place items near the bottom of the page (y0 close to 1) instead of
    // near the top (y0 close to 0), breaking the monotonic ordering.
    //
    // We check that the median y0 of the first half of phrases is less than
    // the median y0 of the second half — a robust signal that the page reads
    // from top to bottom rather than in reverse.
    expect(phrases.length).toBeGreaterThanOrEqual(2);
    const midpoint = Math.floor(phrases.length / 2);
    const firstHalfMidY = phrases.slice(0, midpoint).map((p) => p.y0);
    const secondHalfMidY = phrases.slice(midpoint).map((p) => p.y0);
    const avgFirst = firstHalfMidY.reduce((a, b) => a + b, 0) / firstHalfMidY.length;
    const avgSecond = secondHalfMidY.reduce((a, b) => a + b, 0) / secondHalfMidY.length;
    expect(avgFirst).toBeLessThan(avgSecond);
  });

  it("first phrase is near the top-left visual corner (y0 < 0.2, x0 < 0.2)", async () => {
    const { extractPagePhrases } = await import("./pdf-words");
    const { phrases } = await extractPagePhrases(ROTATED_FIXTURE_PATH, ROTATED_FIXTURE_FILE_ID, 1);

    // The topmost phrase in the fixture is "SIGN TYPE LEGEND" placed at visual
    // y ≈ 50pt on a 612pt-tall viewport → normalised y0 ≈ 0.08.
    // If the rotation path were broken (e.g., coordinates not converted through
    // convertToViewportPoint), the y values would remain in PDF user space
    // (origin bottom-left, y up), placing this phrase near y0 ≈ 1 instead of 0.
    const first = phrases[0]!;
    expect(first.y0).toBeLessThan(0.2);
    expect(first.x0).toBeLessThan(0.2);
  });

  it("produces correct schedule entries and specs through the full pipeline on the rotated page", async () => {
    const { extractPagePhrases } = await import("./pdf-words");
    const { phrases, pageWidth, pageHeight } =
      await extractPagePhrases(ROTATED_FIXTURE_PATH, ROTATED_FIXTURE_FILE_ID, 1);
